...
<link rel="stylesheet" href="{{ url('Backend/css/programs_duration.css') }}">
  <div class="whole_form">
    <div class="heading"><h3>Visa Checklist</h3></div>
    <table id="dataTable">
     @if (!empty($visa_checklist) && count($visa_checklist))
      <thead>
        <tr>
        ...
        <tbody>
            ...
            @foreach ($visa_checklist as $list)
            <form action="{{ route('admin.checklist.update') }}" method="POST">
                @csrf
                <tr>
                    <td style="text-align:center;">{{ $loop->iteration }}</td>
                    <input type="hidden" name="id" value="{{ $list->id }}">
                    <td>
                        <input type="text" name="list_text" value="{{ $list->list_text }}" />
                    </td>
                    <td>
                        <div class="actions">
                            <button type="submit"><i style="color: rgb(2, 112, 207);" class="fa-regular fa-floppy-disk"></i></button>
                            <a href="{{ route('admin.checklist.delete', $list->id) }}"><button type="button"><i class="fa-solid fa-trash-can"></i></button></a>
                        </div>
                    </td>
                </tr>
            </form>
       ...
       <thead>
        <tr>
          <th style="width:50px; text-align:center;">No</th>
          <th>Add Check List</th>
        </tr>
      </thead>
      <tbody>
        <form action="{{route('admin.checklist.create')}}" method="POST">
           @csrf
          <tr>

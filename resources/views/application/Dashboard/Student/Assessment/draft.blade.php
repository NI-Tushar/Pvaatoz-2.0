...
@section('breadcrumb', 'Draft Assessment')
@section('previous-menu', 'Assessment')
@section('active-menu', 'Draft Assessment')
                            ...
                            <tbody>
                                @forelse ($assessments as $assessment)
                                    <tr>
                                        ...
                                        <td>{{ $assessment->duration }} Days</td>
                                        <td>{{ $assessment->consultant->name }}</td>
                                        <td>{{ $assessment->status }}</td>
                                        <td>
                                            <div class="d-flex gap-2">
                                                <button class="btn btn-outline-primary">Send</button>
                                                <form class="dropdown-item" action="{{ route('student.assessment.destroy', $assessment->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
